...
zha = pd.read_excel('.\origin\渣成分表.xlsx')
#liao_comp = pd.read_excel('.\origin\上料成分表.xlsx')
#liao_real = pd.read_excel('.\origin\上料实绩表.xlsx')
#liao_mass = pd.read_excel('.\origin\上料质量表.xlsx')
...
new_iron_comp = iron_comp.pivot_table(index='铁次号', 
              ...
              aggfunc = np.mean)
new_iron_comp.to_excel('new铁水成分表.xlsx')
...
new_iron_real.to_excel('new铁水实绩表.xlsx')
...
new_zha = zha.pivot_table(index='铁次号', 
              columns='采集项名称', values='采集项值',
              aggfunc = np.mean)
new_zha.to_excel('new炉渣成分表.xlsx')


# 大合并---未完成
out = pd.merge(new_iron_comp, new_iron_real, 
